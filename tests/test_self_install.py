...
def test_install_self_targets_every_supported_agent(tmp_path: Path) -> None:
    ...
    assert tmp_path / ".claude/skills/shskill/SKILL.md" in installed
    assert tmp_path / ".codex/skills/shskill/SKILL.md" in installed
    assert all(
...
def test_install_self_is_idempotent_and_updates_owned_file(tmp_path: Path) -> None:
    ...
    assert "shskill install <skill-name>" in target.read_text(encoding="utf-8")
